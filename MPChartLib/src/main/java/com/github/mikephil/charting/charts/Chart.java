...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            // check bounds
            if (!mViewPortHandler.isInBounds(pos[0]))
                continue;
     ...
     */
    public static final int PAINT_GRID_BACKGROUND = 3;

    /**
     * paint for the info text that is displayed when there are no values in the
     * chart
     */
    public static final int PAINT_INFO = 0;
...
//        Bitmap returnedBitmap = Bitmap.createBitmap(getWidth(), getHeight(), Bitmap.Config.RGBA_F16);
//        // Bind a canvas to it
